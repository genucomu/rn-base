# Servicios y Hooks de React Query para API

## Contexto / Problema

La app no tiene implementados los servicios de integración con la API ni los hooks de TanStack Query para consumir dichos servicios. La API está documentada en http://192.168.1.39:3000/api/docs#/Tasks y expone endpoints para gestionar recursos (tasks, groups, subtasks). Actualmente el estado de servidor no está conectado a ningún backend real. Para que la app funcione correctamente se necesita el manejo completo de:
- Groups (creación, edición, gestión de miembros, eliminación)
- Tasks (dentro de grupos)
- Subtasks (dentro de tasks)
- Relaciones entre grupos, miembros, tareas y subtareas

## Objetivos

- Implementar los servicios de API (`src/lib/api/`) que abstraen las llamadas HTTP al backend.
- Crear hooks de TanStack Query (`src/hooks/`) que expongan los queries y mutations para las operaciones de la API (Groups, Tasks, Subtasks).
- Configurar el cliente HTTP con soporte para autenticación (token) y manejo de errores.
- Implementar el modelo de datos completo: Groups → Tasks → Subtasks con Members.
...
- La API expone endpoints RESTful para Groups, Tasks y Subtasks según la documentación en Swagger.
...
- Groups contienen Tasks, Tasks contienen Subtasks.
- Groups tienen Members (usuarios asociados al grupo).
- Las relaciones se manejan mediante IDs (groupId en Task, taskId en Subtask, groupId en Member).
...
- [ ] **RF-3: Servicio de Tasks** — Crear `src/lib/api/tasks.ts` con funciones para:
...
- [ ] **RF-4: Servicio de Subtasks** — Crear `src/lib/api/subtasks.ts` con funciones para:
...
- [ ] **RF-5: Hook de queries de Groups** — Crear `src/hooks/use-groups.ts` con hooks `useGroups`, `useGroup` (por ID) usando `useQuery`.
- [ ] **RF-6: Hook de mutations de Groups** — Crear `src/hooks/use-group-mutations.ts` con hooks `useCreateGroup`, `useUpdateGroup`, `useDeleteGroup`, `useAddGroupMember`, `useRemoveGroupMember`.
- [ ] **RF-7: Hook de queries de Tasks** — Crear `src/hooks/use-tasks.ts` con hooks `useTasks` (por groupId), `useTask` (por ID) usando `useQuery`.
- [ ] **RF-8: Hook de mutations de Tasks** — Crear `src/hooks/use-task-mutations.ts` con hooks `useCreateTask`, `useUpdateTask`, `useDeleteTask`.
- [ ] **RF-9: Hook de queries de Subtasks** — Crear `src/hooks/use-subtasks.ts` con hooks `useSubtasks` (por taskId), `useSubtask` (por ID) usando `useQuery`.
- [ ] **RF-10: Hook de mutations de Subtasks** — Crear `src/hooks/use-subtask-mutations.ts` con hooks `useCreateSubtask`, `useUpdateSubtask`, `useDeleteSubtask`.
- [ ] **RF-11: Constante de API_BASE_URL** — Definir `API_BASE_URL` en `src/constants/api.ts` (o similar) para centralizar la configuración.
- [ ] **RF-12: Tipos TypeScript** — Definir interfaces en `src/lib/api/types.ts` para `Group`, `Task`, `Subtask`, `Member` y sus respectivos Input types según la documentación de la API.
...
- Detalle de grupo con lista de tareas
- Detalle de tarea con lista de subtareas
- Formularios de creación/edición para groups, tasks y subtasks
...
| 403 (forbidden) | El cliente lanza error. Ocurre cuando el usuario intenta acceder/eliminar un grupo o tarea que no le pertenece. |
| 404 (recurso no encontrado) | El cliente lanza error. Los hooks `useGroup`, `useTask`, `useSubtask` deben manejar este caso (opcional: mostrar estado "no encontrado"). |
...
| Query con ID vacío | `useGroup(id)`, `useTask(id)`, `useSubtask(id)` usan `enabled: !!id` para no ejecutar query hasta que haya ID válido. |
...
- **Pruebas manuales** (requiere backend accesible):
  ...
  7. Usar `useTasks(groupId)` → debe listar tareas de un grupo.
  8. Usar `useCreateTask` → debe crear tarea en grupo y refrescar lista.
  9. Usar `useUpdateTask` → debe actualizar tarea y refrescar lista/detalle.
  10. Usar `useDeleteTask` → debe eliminar tarea y refrescar lista.
  11. Usar `useSubtasks(taskId)` → debe listar subtareas de una tarea.
  12. Usar `useCreateSubtask` → debe crear subtarea y refrescar lista.
  13. Usar `useUpdateSubtask` → debe actualizar subtarea y refrescar lista/detalle.
  14. Usar `useDeleteSubtask` → debe eliminar subtarea y refrescar lista.
  15. Verificar que el token se envía en header (inspeccionar network).
  16. Simular 401 → debe activar interceptor global (logout).
  17. Simular 403 → debe mostrar error de permisos.
...
- **Estructura de entidades**: ¿Los campos exactos de Group, Task, Subtask y Member son los asumidos? Revisar docs de Swagger.
- **Cascada de eliminación**: ¿El backend permite eliminar grupos con tareas y tareas con subtareas, o requiere eliminar hijos primero?
...
### Tipos TypeScript
- [ ] Los tipos en `src/lib/api/types.ts` definen interfaces para `Group`, `Task`, `Subtask`, `Member` y sus Input types.
...
- [ ] El hook `useSubtasks(taskId)` usa `useQuery` con `queryKey: ['subtasks', taskId]` y `enabled: !!taskId`.
...
- [ ] Los hooks de mutations de subtasks (`useCreateSubtask`, `useUpdateSubtask`, `useDeleteSubtask`) invalidan queries relevantes en `onSuccess`.
